...
class Fish {
  constructor(fishData) {
    ...
    this.incompleteRanges = [];
    {
      ...
      this.dailyDuration = dateFnsExt.normalizeDuration({
  ...
  isCatchable(fe = false, dt = null) {
    ...
    if (crs.length > 0) {
      return dateFnsExt.isSameOrAfter(dt, eorzeaTime.toEarth(+crs[0].start));
    }
  ...
  availableRangeDuring(r, fe = false) {
    ...
    var m = +r.start;
    let o = [];
    var rs = dateFnsUtc.addMinutes(dateFnsUtc.setHours(m, this.startHour), (this.startHour % 1) * 60);

    if (this.endHour < this.startHour) {
      // Available times wraps around date...
      if (dateFnsUtc.getHours(m) < this.endHour) {
        ...
        o.push(dateFnsExt.intervalAfter(dateFnsUtc.subDays(rs, 1), d));
        if (dateFnsUtc.getHours(+r.end) > this.startHour) {
          // Also include the portion of the window when the fish is available once again.
          o.push(dateFnsExt.intervalAfter(rs, d));
        }
      } else {
        o.push(dateFnsExt.intervalAfter(rs, d));
      }
    } else if (dateFnsUtc.getHours(m) < this.endHour) {
  ...
  addCatchableRange(nextRange) {
    ...
    var merged = dateFnsExt.intervalXor(lastRange, nextRange);
    // SAFEGUARD:
